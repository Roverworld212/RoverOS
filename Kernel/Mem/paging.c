...
void mapUI(){
    printq("%qMapping UI\n", QINFO);
    uint64 end = kvinf.video.physical+(200*PAGE_SZ);
    for(uint64 start = kvinf.video.physical; start <= end; start+=PAGE_SZ){
        mapPage(&kspace, start, start, true, false, true);
    }
    printq("%qMapped UI\n", QINFO);
    return;
}

void mapRegion(uint64 start, uint64 end, bool w, bool user, bool nx){
    start = ALIGN(start);
    end = ALIGN(end);
    for(; start <= end; start+=PAGE_SZ){
        mapPage(&kspace, start, start, w, user, nx);
    }
}